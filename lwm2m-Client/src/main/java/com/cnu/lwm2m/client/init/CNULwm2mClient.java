...
@Slf4j
@Component
//@PropertySource("lwm2m-client.properties")
@PropertySources({
	@PropertySource("classpath:lwm2m-client.properties"),
	@PropertySource(value="file:lwm2m-client.properties", ignoreResourceNotFound=true)
})
public class CNULwm2mClient extends AbsCNUModelSettings implements DisposableBean {
	...
	public void run() {
		...
		String endpoint = "CNU0" + Math.round(Math.random() * 1000000000);
	...
	private NetworkConfig createConfig() {
		try {
			Resource resource = resourceLoader.getResource("classpath:Californium.properties");
